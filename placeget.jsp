...
try{
	Connection conn=GetConnection.Establish();
		if(conn!=null)
		{
			...
			<% while(rs.next()){
	          %>
	          <form action="deletePlace.jsp">
	          <tr>
	              <td><%=rs.getString("branchID") %>
	              <input type="hidden" name="branchID" value="<%=rs.getString("branchID") %>"><br></td>
	              <br>
	              <td><%=rs.getString("branchName") %>
	              <br>
	  ...
	  <%}%>
		      ...
		      <br>
                    ...
                    <td style="text-align:center;color: white;"><a href="addPlace.jsp"><button>Add New Place</button></a></td>
		      </div>
      <%}
	else
